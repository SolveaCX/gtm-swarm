#!/bin/bash
# AI 早报（每早 10:10；launchd com.477.loop-ai-briefing；自产,不依赖外部系统）
...
PROMPT="$HOME/.477-automation/prompts/ai-briefing-prompt.md"
LOGDIR="$HOME/Library/Logs/daily-ai-briefing"; mkdir -p "$LOGDIR"
T="$(date +%F)"; LOG="$LOGDIR/$T.log"
REPORT="$VAULT/00_Inbox/${T}_AI早报.md"
...
python3 "$HOME/.477-automation/flatkey-agent.py" \
  --out "$REPORT" \
  --writer "$HOME/.477-automation/writers/briefing-writer.md" \
  --queries "AI news today 2026" "Anthropic OpenAI Google AI announcement 2026" "AI model release funding 2026" "AI policy chip 2026" \
  --recency-days 4 --scrape 5 --max-steps 14 >>"$LOG" 2>&1
RC=$?
