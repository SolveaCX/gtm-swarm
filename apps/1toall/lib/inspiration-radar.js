...
信源权威度按「信源等级」字段判断，直接影响 evidence 维度：官方（模型厂商/平台一手发布）与创始人/高管（CEO、创始人、投资人，讲的是自己公司的真实决策与数据）最可信，evidence 给足；一线负责人次之；AI builder（独立开发者/实践者）讲的是自己跑出来的一手经验，**只要写清了做法和结果就照常给分，不要因为他不是大厂/创始人就压分**；媒体/社区是二手转述，需要交叉验证，evidence 最低。判断依据是「有没有真实做过、给没给得出可核验的过程与结果」，不是发言人的头衔。${brief ? `\n${brief}\n给切口和钩子时必须站在上面账号的定位与风格上——写清这个账号该用什么姿势接这条素材，不要泛泛的媒体建议。` : ''}`;
