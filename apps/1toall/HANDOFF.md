...
**账号后台**：账号页不再实时拉钉钉——数据存系统自库（`acct-stats` 集合，
`/api/accounts/board` CRUD + `/import` 幂等导入），历史钉钉数据一次性搬入后在页面
手动维护；发布连接器上线后改为自动回流。品牌卡新增「生产渠道」区：每条渠道即一份
生产规格书（画幅/时长/交付物/调用的 skill），点开看全文——skill 本体装在产能机上。
