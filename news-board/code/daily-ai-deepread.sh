#!/bin/bash
# AI精读（每早 10:20；launchd com.477.loop-ai-deepread；自产,不依赖外部系统）
...
PROMPT="$HOME/.477-automation/prompts/ai-deepread-prompt.md"
LOGDIR="$HOME/Library/Logs/daily-ai-deepread"; mkdir -p "$LOGDIR"
T="$(date +%F)"; LOG="$LOGDIR/$T.log"
REPORT="$VAULT/00_Inbox/${T}_AI精读.md"
...
python3 "$HOME/.477-automation/flatkey-agent.py" \
  --out "$REPORT" \
  --writer "$HOME/.477-automation/writers/deepread-writer.md" \
  --queries "AI deep analysis essay 2026" "AI industry deep dive Stratechery Latent Space 2026" "AI thoughtful long read 2026" \
  --recency-days 7 --scrape 3 --max-steps 12 >>"$LOG" 2>&1
RC=$?
